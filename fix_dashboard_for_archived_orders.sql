-- ============================================
-- Corregir Dashboard para Órdenes Archivadas
-- ============================================
-- Las órdenes con status 'Pagado' SIEMPRE están en orders_archive
-- Este script:
-- 1. Migra todas las órdenes de orders_archive al dashboard
-- 2. Asegura que el trigger funcione correctamente
-- ============================================
...
BEGIN
    RAISE NOTICE '========================================';
    RAISE NOTICE 'CORRIGIENDO DASHBOARD PARA ÓRDENES ARCHIVADAS';
    RAISE NOTICE 'Restaurant ID: %', v_restaurant_id;
    RAISE NOTICE '========================================';
    RAISE NOTICE '';

    -- PASO 1: Verificar órdenes en orders_archive
    SELECT COUNT(*) INTO v_paid_orders_count 
    FROM orders_archive 
    WHERE status = 'Pagado' 
    AND restaurant_id = v_restaurant_id;
    
    RAISE NOTICE 'PASO 1: Verificación';
    RAISE NOTICE '  Órdenes pagadas en orders_archive: %', v_paid_orders_count;
    RAISE NOTICE '';
    ...
    -- PASO 2: Migrar todas las órdenes de orders_archive al dashboard
    RAISE NOTICE 'PASO 2: Migrando eventos desde orders_archive...';
    ...
    -- PASO 3: Crear resúmenes diarios
    RAISE NOTICE 'PASO 3: Creando resúmenes diarios...';
    ...
    RAISE NOTICE '  Eventos totales: %', v_events_inserted;
    RAISE NOTICE '  Resúmenes totales: %', v_summary_inserted;
    RAISE NOTICE '';
...
SELECT 
    ...
    (SELECT COUNT(*) FROM orders_archive WHERE status = 'Pagado' AND restaurant_id = '2e0110b2-5977-4cef-b987-49afddd1795d') as ordenes_pagadas_en_archive,
